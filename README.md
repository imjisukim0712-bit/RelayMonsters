...
| 전투 8단계 턴 시퀀스 · 14종 트리거 · 트리거 우선순위 · 재귀 깊이 제한 | ✅ |
| 몬스터 66종 (Lv1/Lv3/Lv5 능력 198문구, 전원 고유) | ✅ |
| 마나 시스템 — T2~T6 티어별 마나 유닛 1종 · 고유 트리거 마나 획득/소비 · T4 연금술사 마나 공급 | ✅ |
| 66종 전신 벡터 SVG 아트 + 실루엣 식별자 | ✅ |
...
src/
  ...
  data/    species.js     로스터 66종 + 능력 데이터
  ...
  art/     symbols.js     66종 SVG symbol 시트
           items.js       소모품 아이콘
...
- **피닉스 재소환** — 사망 시 `summon(self)` 으로 자신을 다시 소환한다. 원래 최대 체력은
  유지한 채 체력 40/70/100%로 새 개체가 등장하며, 새 개체는 능력을 물려받지 않아
  같은 전투에서 반복 재소환되지 않는다 (슬라임 분열과 같은 소환 경로를 공유).
- **소환 계열 확장** (다람쥐지기·포자버섯·네크로맨서·슬라임 여왕·주작·본드래곤) — `doSummon()` 이
  `effect.count` (한 번에 여러 기 소환, 매 기마다 링 만석 여부를 다시 확인) 와 `effect.scaleAtk`
  (자기 재소환 시 공격력도 비율만큼 줄이는 옵션, 본드래곤의 반쪽 스탯 분열용)를 지원하도록
  일반화됐다. 유닛이 소환될 때마다 아군에게는 `ON_ALLY_SUMMON`, 상대 팀에게는
  `ON_ENEMY_SUMMON` 트리거가 발동해 소환 자체에 반응하는 능력을 만들 수 있다
  (재귀 소환 연쇄는 기존 트리거 재귀 깊이 제한을 그대로 공유해 안전하다).
