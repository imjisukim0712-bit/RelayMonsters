...
import { abilityLines, abilityLine } from '../engine/text.js';
...
export const ICONS = { atk: ICO_ATK, hp: ICO_HP, shield: ICO_SHIELD };

// 공격력·체력·보호막 아이콘은 서로 다른 형태를 쓰고 항상 숫자를 병기한다 (13.3)
export function statChips({ atk, hp, maxHp, shield = 0 }) {
...
export function detailPanel(u, { extra = '' } = {}) {
  ...
  const main = abilities.find((a) => a.source === 'species');
  const granted = abilities.filter((a) => a.source === 'granted');

  const lines = main
    ? abilityLines(main).map(
  ...
  return `<div class="detail" style="--tier:${color}">
    <div class="detail-head">
      ...
      <div class="detail-meta">
        <div class="detail-title"><b>${sp.name}</b><span class="tier-tag">T${sp.tier}</span><span class="lv-tag">Lv${level}</span></div>
        <div class="detail-stats">${statChips({ atk: unitAtk(u), hp: unitHp(u) })}</div>
        <div class="exp-row">
